...
#include "../Include/cholmod_core.h"
...
#define PATTERN
#include "t_cholmod_dense_scale.c"
#define REAL
#include "t_cholmod_dense_scale.c"
#define COMPLEX
#include "t_cholmod_dense_scale.c"
...
cholmod_sparse *CHOLMOD(dense_to_sparse_scaling)
(
    /* ---- input ---- */
...
)
{
    ...
    {
	case CHOLMOD_REAL:
	    C = r_cholmod_dense_to_sparse_scale(X, values, Common,Scale_factor) ;
	    break ;

	case CHOLMOD_COMPLEX:
	    C = c_cholmod_dense_to_sparse_scale(X, values, Common,Scale_factor) ;
	    break ;

	case CHOLMOD_ZOMPLEX:
	    C = z_cholmod_dense_to_sparse_scale(X, values, Common,Scale_factor) ;
	    break ;
    }
    return (C) ;
}

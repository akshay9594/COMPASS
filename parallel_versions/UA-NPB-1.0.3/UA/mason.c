...
void mortar()
{
  ...
  nr_init_omp((int *)idmo, n1, -1);
  nr_init_omp(nemo, n2, -1);
  nr_init_omp((int *)vassign, n2, -1);
  nr_init_omp((int *)emo, n3, -1);
  ...
  count = front[nelt-1]+newc[nelt-1];
  nvertex = count;
...
static void get_emo(int ie, int n, int ng)
{
  ...
  if (!L1) {
    omp_set_lock(&tlock[n]);
    ntemp = nemo[n]+1;
    nemo[n] = ntemp;
    emo[n][ntemp][0] = ie;
    emo[n][ntemp][1] = ng;
    omp_unset_lock(&tlock[n]);
  }
}

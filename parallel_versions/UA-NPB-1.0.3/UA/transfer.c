...
void transf(double tmor[], double tx[])
{
  ...
  for (ie = 0; ie < nelt; ie++) {
    for (iface = 0; iface < NSIDES; iface++) {
      ...
      //  printf("thread = %d , il1= %d\n", omp_get_thread_num() , il1);
      //  printf("thread = %d , il2= %d\n", omp_get_thread_num() , il2);
      //  printf("thread = %d , il3= %d\n", omp_get_thread_num() , il3);
      //  printf("thread = %d , il4= %d\n", omp_get_thread_num() , il3);
...
void transfb(double tmor[], double tx[])
{
  ...
  for (ie = 0; ie < nelt; ie++) {
    for (iface = 0; iface < NSIDES; iface++) {
      ...
      //omp_set_lock(&tlock[ig1]);
      tmor[ig1] = tmor[ig1]+tx[il1]*third;
      //omp_unset_lock(&tlock[ig1]);

      //omp_set_lock(&tlock[ig2]);
      tmor[ig2] = tmor[ig2]+tx[il2]*third;
      //omp_unset_lock(&tlock[ig2]);

      //omp_set_lock(&tlock[ig3]);
      tmor[ig3] = tmor[ig3]+tx[il3]*third;
      //omp_unset_lock(&tlock[ig3]);

      //omp_set_lock(&tlock[ig4]);
      tmor[ig4] = tmor[ig4]+tx[il4]*third;
      //omp_unset_lock(&tlock[ig4]);
        ...
        for (ije1 = 0; ije1 < nnje; ije1++) {
          ...
          for (ije2 = 0; ije2 < nnje; ije2++) {
            ...
            for (col = 2-shift-1; col < LX1-shift; col++) {
              ...
              //omp_set_lock(&tlock[ig]);
              tmor[ig] = tmor[ig]+temp[ije1][col][v_end[ije2]]*0.5;
              //omp_unset_lock(&tlock[ig]);

    ...
    )
{
    ...
    Int *Super, *Head, *Ls, *Lpi, *Lpx, *SuperMap, *RelativeMap, *Next,
...
#pragma omp parallel for num_threads(CHOLMOD_OMP_NUM_THREADS)   \
    schedule (dynamic) if ( pend - psx > 1024 )
...
//     num_threads(CHOLMOD_OMP_NUM_THREADS) if ( k2-k1 > 64 )schedule(dynamic)
        for (k = k1 ; k < k2 ; k++)
        {
            if (stype != 0)
            {
	...
	gettimeofday(&st, NULL);
		...
		double blastime = (ed.tv_sec + (double)ed.tv_usec/1000000) - (st.tv_sec + (double)st.tv_usec/1000000);       
		...
		if (ndrow3 > 0)
                ...
                DEBUG (CHOLMOD(dump_real) ("C", C, ndrow2, ndrow1, TRUE,
                  ...
                  if(Ls[pdi1 + ndrow2 -1] > Ls[psi+nsrow-1] && 
                    Ls[pdi1] < Ls[0])
                      break;

#pragma omp parallel for num_threads(CHOLMOD_OMP_NUM_THREADS)   \
    if ( ndrow2 > 64 )schedule(dynamic)
                for (i = 0 ; i < ndrow2 ; i++)
                {
...
//     num_threads(CHOLMOD_OMP_NUM_THREADS) if (ndrow1 > 64)schedule(dynamic)
                ...
                double subsubtime = (stop.tv_sec + (double)stop.tv_usec/1000000) - (init.tv_sec + (double)init.tv_usec/1000000);       
	...
	double lapacktime = (terminate.tv_sec + (double)terminate.tv_usec/1000000) - (begin.tv_sec + (double)begin.tv_usec/1000000);       
	...
	printf("Total application time in seconds= %lf s\n", sum+sum1+sum2+sum3);
